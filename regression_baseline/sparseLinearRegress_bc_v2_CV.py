...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 13 15:20:13 2021

@author: guo.1648
"""

# version 2 for 10-fold Cross Validation:
...
dstRootDir = '/eecf/cbcsl/data100b/Chenqi/project_brain/regression_baseline/results/DSM_Anxi_T/sparseLinearRegress_bc_v2_CV/'

threshold = 0.05 # threshold for connData



def loadDataTarget(train_test_root, flag):
    # flag = 'train' or 'test'
    print('@@@ loading ...')
...
if __name__ == '__main__':
    ...
    for i in range(10): # for each CV fold
        ...
        #"""
        conn_list_train, behavior_val_list_train = loadDataTarget(train_test_root, flag = 'train')
        conn_list_test, behavior_val_list_test = loadDataTarget(train_test_root, flag = 'test')
        #"""
        ...
        for i in range(train_itr_num):
            ...
            f_pkl = open(dstDir+'checkpts/train_iter'+str(i)+'.pkl', 'wb')
...
"""
# just to compute mean & std metrics:
print('^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Metrics:')
acc_test_list = [0.5686274509803921, 0.6078431372549019, 0.5841584158415841,
                 0.5686274509803921, 0.5294117647058824, 0.5980392156862745,
                 0.5544554455445545, 0.5742574257425742, 0.5098039215686274,
                 0.43]
...
mae_test_list = [0.5063954413404801, 0.4647688155465994, 0.5023164339204617,
                 0.4775834934678637, 0.4615489402491714, 0.467969809253838,
                 0.4644095984403111, 0.5047250555061408, 0.4679729213294637,
                 0.5047129320392585]
mae_test_mean = np.mean(mae_test_list)
mae_test_std = np.std(mae_test_list)
print('mae_test_mean = {:.4f}'.format(mae_test_mean))
print('mae_test_std = {:.4f}'.format(mae_test_std)) 
"""
