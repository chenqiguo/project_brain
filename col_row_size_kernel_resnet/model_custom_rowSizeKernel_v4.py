...
# version 4: based on version 2, using resnet9 architecture with row-size kernel.

# referenced from model_custom_rowSizeKernel_v2.py

# instead of simply using resnet, here we construct our own model architecture:
...
def Model_modi_resnet9(row_size) -> Model: #**kwargs: Any
    
    return _model(BasicBlock, [1, 1, 1, 1], row_size) #,**kwargs
...
if __name__ == '__main__':
    input_width = 379
    model = Model_modi_resnet9(input_width).cuda()
    ...
    connData_ = connData.float().unsqueeze(1).cuda(non_blocking=True) # connData_.size(): torch.Size([batch_size, 1, 64984, 379])
