...
# version 3: based on version 2, use rowKernel -> colKernel -> rowKernel -> colKernel -> ...
...
class BasicBlock_colKer(nn.Module): 
    ...
    def forward(self, x):
        ...
        if self.kernel_size != self.planes:
            ...
            print('planes = ' + str(self.planes))
            """
            if 2*self.kernel_size == self.planes: # for layer3
                identity = torch.cat((x,x), 2)
            elif self.kernel_size > self.planes: # for layer1
                identity = x[:,:,:self.planes,:]
...
class Model(nn.Module):
    def __init__(self,
        ...
        self.relu = nn.ReLU(inplace=True) # should output 64 number of (2V x 1) tensors!
        # NO nn.MaxPool2d
        self.layer1 = self._make_layer_col(block_col, 64, self.col_size, layers[0]) # (block, planes, kernel_size, num_blocks)
        self.layer2 = self._make_layer_row(block_row, 128, 64, layers[1])
        self.layer3 = self._make_layer_col(block_col, 128, 64, layers[2])
        self.layer4 = self._make_layer_row(block_row, 256, 128, layers[3]) # modification: How about using 64 channels kernels all the time???
        
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.flatten = nn.Flatten() # check: should output torch.Size([batch_size, 256])
        self.fc = nn.Linear(256 * block_row.expansion, 1) # for regression!!!
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        out = self.layer2(out)
        out = self.layer3(out)
        out = self.layer4(out) # check: torch.Size([batch_size, 1, 128, 256])
        
        out = out.permute(0, 3, 2, 1) # check: torch.Size([batch_size, 256, 128, 1])
        
        out = self.avgpool(out) # check: torch.Size([batch_size, 256, 1, 1])
        out = self.flatten(out) # check: torch.Size([batch_size, 256])
...
def Model_modi_resnet18(row_size, col_size) -> Model: #**kwargs: Any
    
    return _model(BasicBlock_rowKer, BasicBlock_colKer, [2, 2, 2, 2], row_size, col_size) #,**kwargs
