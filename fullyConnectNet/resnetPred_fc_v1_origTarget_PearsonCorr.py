...
# use Pearson Correlation instead of L1 loss as metric!
...
def epoch_end(epoch, result):
    print("Epoch [{}], last_lr: {:.5f}, train_loss: {:.4f}, train_corr: {:.4f}, val_loss: {:.4f}, val_corr: {:.4f}".format(
        epoch, result['lrs'][-1], result['train_loss'], result['train_corr'], result['val_loss'], result['val_corr']))
...
def fit_one_cycle(epochs, tolerance, max_lr, model, train_loader, val_loader, 
    ...
    for epoch in range(epochs):
        ...
        for batch in train_bar:
            ...
            loss, corr = training_step(batch, tolerance, model)
        ...
        result['train_corr'] = torch.stack(train_corrs).mean().item()
        result['lrs'] = lrs
        ...
        if result['val_corr'] >= best_validCorr:
            best_validCorr = result['val_corr']
            torch.save(model.state_dict(), result_dir+'model_weights_bestValidCorr.pth')
