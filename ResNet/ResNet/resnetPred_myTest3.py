...
if __name__ == '__main__':
    ...
    model = Model().cuda()
    #model = resnet50(pretrained=False) # for debug
    ...
    for epoch in range(1, epochs + 1):
        ...
        # newly modified:
        data_frame.to_csv('results_myTest3/{}_statistics.csv'.format(save_name_pre), index_label='epoch')
        
        #"""
        # USED code: only save the "best" model:
        if valid_loss <= best_validLoss:
            best_validLoss = valid_loss
            torch.save(model.state_dict(), 'results_myTest3/best_{}_model.pth'.format(save_name_pre))
        #"""
        """
